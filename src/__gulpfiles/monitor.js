...
function monitorSource() {
	...
	watch(['hdsdk/**/*.ts', 'hdfmc/**/*.ts', 'hdlogger/**/*.ts', 'hdautopilot/**/*.ts'], {ignoreInitial: false}, series(buildTasks.buildB78XH, rollupTasks.rollupHDSDK, rollupTasks.rollupHDLogger, rollupTasks.rollupHDFMC, cleanTasks.cleanBuild, distributeTasks.distribute)).on('change', function (path, stats) {
