...
export class CJ4_FMC_PilotWaypoint_Manager {
	private _fmc: B787_10_FMC;
	private _pilotWaypointArray: any[];
	private _pilotWaypointCount: number;
	private _pilotWaypointArray1: any[];
	private _pilotWaypointArray2: any[];
	private _pilotWaypointArray3: any[];
	private _pilotWaypointArray4: any[];
	private _pilotWaypointArray5: any[];
...
class CJ4_FMC_PilotWaypointPage {
	private readonly _fmc: any;
	private _isDirty: boolean;
	private _currentPage: number;
	private _pageCount: number;
	private _rows: any[];
	private _selectedPilotWaypointIndex: any;
	private _showPilotWaypointPage: any;
	private _tempWaypointParameters: { lo: any; pbd: any; la: any; ident: any; exists: boolean };

	constructor(fmc: B787_10_FMC, defineWaypoint = false) {
		this._fmc = fmc;
	...
	bindEvents() {
		if (this._showPilotWaypointPage) {
			if (this._selectedPilotWaypointIndex >= 0) {
				...
				this._fmc._renderer.lsk(1).event = async () => {
				...
				this._fmc._renderer.lsk(4).event = () => {
					const inputValue = this._fmc.inOut;
				...
				this._fmc._renderer.lsk(5).event = async () => {
					const inputValue = this._fmc.inOut;
					if (inputValue.length > 0) {
