...
declare class BaseInstrument extends TemplateElement {
	...
	requestCall(_func: Function, _timeout: number = 0);
...
declare class SimVar {
	...
	static GetGameVarValue(name: string, unit: string, param1: number = 0, param2: number = 0): any
...
declare class Simplane {
	...
	static getAutoPilotAltitudeLockValue(_units?: string = 'feet'): number

	static getAutoPilotSelectedAltitudeLockValue(_units?: string = 'feet')
...
declare function RegisterViewListener(name, callback?: Function = null, requiresSingleton?: boolean = false)
...
declare class RadioNav {
	...
	getBestILSBeacon(_useNavSource = UseNavSource.YES_ONLY): NavBeacon

	getILSActiveFrequency(_index: number): number;

	setILSActiveFrequency(_index: number, _value: number): number

	getBestVORBeacon(_useNavSource = UseNavSource.YES_ONLY): NavBeacon
...
declare class FMCDataManager {
	...
	ExecuteFlightPlan(fmc: BaseFMC): Promise<boolean>;
}
...
declare class FlightPlanManager {
	...
	getCoordinatesAtNMFromDestinationAlongFlightPlan(distance: any, toApproach: boolean = false): {
		lla: LatLong;
		prevIndex: number;
	};
	...
	setDestinationRunwayIndex(index: number, runwayExtension: number = -1, callback: Function = () => {
	})
...
declare function fastToFixed(_val: number, _fraction: number = undefined): string
...
declare class FMCMessagesManager {
	 ...
	 */
	static createMessage(title, text, priority = 1000): FMCMessage
	 ...
	 * @param {Number} priority
	 */
	showMessage(title, text, priority = 1000): void
...
declare class FMCMessage {
	 ...
	 * @param {Number} priority
	 */
	constructor(title: string, text: string, priority?: number = 1000)
}
...
declare class SpeedDirector {
	constructor(fmc: Boeing_FMC)

	update(flightPhase: number);

	private _fmc: B787_10_FMC;
	_commandedSpeedType: number;
	_lastCommandedSpeedType: number;
	_speedPhase: number;
	_lastSpeedPhase: number;
	_machMode: any;
	_lastMachMode: any;
	_lastSpeed: number;
	_speedCheck: any;
	...
	speed: number;
	machModeActive: boolean;
	_accelerationSpeedRestriction: AccelerationSpeedRestriction;
	_overspeedProtection: OverspeedProtection;
	_climbSpeedRestriction: ClimbSpeedRestriction;
	_climbSpeedTransition: ClimbSpeedTransition;
	_climbSpeedSelected: ClimbSpeed;
	_climbSpeedEcon: ClimbSpeed;
	_cruiseSpeedSelected: CruiseSpeed;
	_cruiseSpeedEcon: CruiseSpeed;
	_descentSpeedRestriction: DescentSpeedRestriction;
	_descentSpeedTransition: DescentSpeedTransition;
	_descentSpeedSelected: DescentSpeed;
	_descentSpeedEcon: DescentSpeed;

	get machModeActive(): any;
	...
	_resolveMachKias(speed: any): any;

	get speed(): any;

	get speedPhase(): number;

	get commandedSpeedType(): number;

	_updateLastSpeed(speed: any): void;

	_updateCheckSpeed(): void;

	update(flightPhase: any): void;

	_updateClimbSpeed(): void;

	_updateCruiseSpeed(): void;

	_updateDescentSpeed(): void;

	_updateApproachSpeed(): void;

	_updateLastCommandedSpeed(): void;

	_updateCommandedSpeed(speedType: any, speedPhase: any): void;

	_updateFmcIfNeeded(): void;
...
declare class CJ4_FMC_PilotWaypointParser {
	...
	public static buildPilotWaypointFromExisting(id, number: number, number2: number, _fmc: B787_10_FMC)

	public static async parseInput(value: string | any, scratchPadWaypointIndex: any, _fmc: B787_10_FMC)

	public static parseInputLatLong(inputValue: any, _fmc: B787_10_FMC)

	public static parseInputPlaceBearingDistance(inputValue: any, _fmc: B787_10_FMC): any;
}
...
declare class FlightPlanSegment {
	...
	constructor(type, offset, waypoints): void
}
...
declare class B787_10_FMC_SelectWptPage {
	public static ShowPage(fmc: B787_10_FMC, waypoints: any[], callback);
};
