		...
		if (this.waypoint) {
			if (this._navMode == Jet_NDCompass_Navigation.NAV) {
				if (_name && _name != '') {
					...
					/**
					 * TRACK is not shown in real life
					 */
					/*
					...
					*/
					...
					if (groundSpeed < 10) {
						...
						if (this.waypointTime != null) {
							const time = (SimVar.GetSimVarValue('E:ZULU TIME', 'seconds') + Simplane.getNextWaypointETA()) % (24 * 3600);
							diffAndSetText(this.waypointTime, Math.floor((time / 3600) % 24).toString().padStart(2, '0') + Math.floor((time / 60) % 60).toString().padStart(2, '0') + '.0z')
						}
					}
				} else {
					if (this.waypointName != null) {
