...
class FacilityLoader {
	...
	async getAirway(intersection, name = '') {
		await this.waitRegistration();
		let airways = [];
		...
		if (intersectionInfo instanceof WayPointInfo) {
			let data = await this.getAirwayData(intersectionInfo, intersectionInfo.routes[i].name, maxLength);
			this.loadedAirwayDatas.set(intersectionInfo.routes[i].name, data);
			let airway = new Airway();
			airway.SetFromIAirwayData(data);
			airways.push(airway);
		}
		return airways;
	}
