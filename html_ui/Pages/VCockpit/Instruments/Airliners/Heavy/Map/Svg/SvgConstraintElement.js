...
class SvgTopOfXCustomElement extends SvgMapElement {
	constructor(name, imageName) {
		super();
		this.imageName = imageName;
		this.heading = 0;
	...
	createDraw(map) {
		...
		container.id = this.id(map);
		container.classList.add('map-city-icon');
		container.setAttribute('width', fastToFixed(map.config.topOfXWaypointIconSize * 2, 0));
		container.setAttribute('height', fastToFixed(map.config.topOfXWaypointIconSize * 2, 0));
		container.setAttributeNS('http://www.w3.org/1999/xlink', 'href', map.config.imagesDir + this.imageFileName());
		return container;
	}
	...
	updateDraw(map) {
		...
		if (isFinite(this.x) && isFinite(this.y)) {
			...
			if (Math.abs(this.x - this._lastX) > 0.1 || Math.abs(this.y - this._lastY) > 0.1) {
				this.svgElement.setAttribute('x', fastToFixed((this.x - map.config.topOfXWaypointIconSize), 1));
				this.svgElement.setAttribute('y', fastToFixed((this.y - map.config.topOfXWaypointIconSize), 1));
				let angle = this.heading;
				if (map.rotateWithPlane) {
					angle -= map.planeDirection;
				}
