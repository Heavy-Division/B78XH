...
class B787_10_FMC_HoldsPage {
	...
	bindFplnHoldInputs(currentHold, numHolds) {
		...
		if (numHolds < 6) {
			this._fmc.onRightInput[4] = () => B787_10_FMC_LegsPage.ShowPage1(this._fmc, true);
		}
	...
	bindHoldListInputs(currentHolds) {
		...
		if (currentHolds.length < 6) {
			this._fmc.onRightInput[5] = () => B787_10_FMC_LegsPage.ShowPage1(this._fmc, true);
		}
	}
	...
	static handleHoldPressed(fmc) {
		...
		if (holds.length === 0) {
			...
			B787_10_FMC_HoldsPage.showHoldPage(fmc);
		} else if (holds.length > 1) {
